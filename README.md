...
- `Moded_HD.HPF`는 더 이상 빌드 필수 입력이 아닙니다.
...
2. 영어 음성 대사 자막까지 포함하려면, 팬이 수정한 HD 아카이브를 `Moded_HD.HPF` 이름으로 같은 폴더에 둡니다.
3. 릴리즈 ZIP을 게임 폴더에 그대로 압축 해제합니다.
4. `patch_and_install.bat`를 실행합니다.
5. 완료 후 `KOREAN.HPF`가 생성되고 `runtime` 파일이 게임 폴더로 자동 복사됩니다.
6. `start.bat`로 실행합니다.

`Moded_HD.HPF`가 없으면 일부 음성 대사 구간은 자막이 비어 있을 수 있습니다.
...
- `<PatchWorkspace>\translation\Moded_HD.HPF`
- `<PatchWorkspace>\Moded_HD.HPF`
...
- `translation/kosubs.user.tsv`: 로컬 `Moded_HD.HPF`에서 추출한 영문+타임코드 템플릿(한글 열은 `subko.tsv`로 자동 병합)
