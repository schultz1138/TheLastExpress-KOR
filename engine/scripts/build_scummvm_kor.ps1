...
if ([string]::IsNullOrWhiteSpace($GamePath)) {
	$GamePath = Join-Path $RepoRoot "The Last Express"
}
if ([string]::IsNullOrWhiteSpace($ConfigPath)) {
	$ConfigPath = Join-Path $WorkspaceRoot "scummvm_kor_test.ini"
}
if ([string]::IsNullOrWhiteSpace($SmokeLogPath)) {
	$SmokeLogPath = Join-Path $WorkspaceRoot "scummvm_kor_smoke.log"
}
...
if (-not $SkipCopyToCustomBuild) {
	Write-Step "Copy runtime binaries"
	...
	if (-not (Test-Path $targetExe)) {
		Fail 80 "Runtime binary copy failed: $targetExe"
	}
...
if (-not $SkipCopyToCustomBuild) {
	Write-Host "Runtime output: $CustomBuildDir"
}
exit 0
