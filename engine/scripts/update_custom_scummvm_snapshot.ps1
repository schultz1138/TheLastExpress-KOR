...
if ([string]::IsNullOrWhiteSpace($SourceRoot)) {
    $SourceRoot = Join-Path $WorkspaceRoot "clean\scummvm-2026.1.0"
}
if ([string]::IsNullOrWhiteSpace($TargetRoot)) {
    $TargetRoot = Join-Path $WorkspaceRoot "snapshots\custom-2026.1.0"
}
...
if (-not (Test-Path $SourceRoot)) {
    Fail 10 "SourceRoot was not found: $SourceRoot"
}

$robocopy = Get-Command robocopy -ErrorAction SilentlyContinue
if (-not $robocopy) {
    Fail 11 "robocopy command was not found"
}
